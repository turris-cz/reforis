...
def updates():
    ...
    if request.method == "GET":
        ...
        if data["enabled"] is True:
            data["user_lists"] = [package["name"] for package in settings["user_lists"] if package["enabled"] is True]
            data["languages"] = [language["code"] for language in settings["languages"] if language["enabled"] is True]
...
def packages_get():
    ...
    response = current_app.backend.perform(
        "updater", "get_package_lists", {"lang": _get_locale_from_backend(current_app)}
    )
